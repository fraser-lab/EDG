...
class TestSubstructurePotential:
    ...
    def test_ensemble_shape_handling(
        self, ensemble_coords, mock_feats, substructure_potential
    ):
        ...
        assert energy.shape == (num_ensembles,), (
            f"Expected energy shape {(num_ensembles,)}, got {energy.shape}"
        )

        # Energy should be finite
        assert torch.isfinite(energy).all(), "Energy should be finite"

    def test_gradient_ensemble_shape_handling(
        self, ensemble_coords, mock_feats, substructure_potential
    ):
    ...
    def test_rmsd_reduction_with_gradient_steps(
        ...
        for step in range(n_steps):
            ...
            grad = potential.compute_gradient_ensemble(
                coords_detached, mock_feats, potential.parameters
            )
        ...
        assert (final_rmsd < initial_rmsd).all(), (
            f"RMSD should decrease for all ensemble members. Initial: {initial_rmsd}, Final: {final_rmsd}"
        )

        # Check that the reduction is significant (at least 10% reduction)
        reduction_ratio = final_rmsd / initial_rmsd
        assert (reduction_ratio < 0.9).all(), (
            f"RMSD reduction should be at least 10%. Reduction ratio: {reduction_ratio}"
        )

    def test_ensemble_members_treated_equally(
        self, ensemble_coords, mock_feats, substructure_potential
    ):
        ...
        identical_coords = (
            coords[:, :1, :, :].expand(-1, ensemble_size, -1, -1).contiguous()
        )

        # Compute gradients using the potential's method
        grad = potential.compute_gradient_ensemble(
            identical_coords, mock_feats, potential.parameters
        )
        ...
        for ensemble_idx in range(num_ensembles):
            ...
            for member_idx in range(1, ensemble_size):
                ...
                assert max_diff < 1e-6, (
                    f"Ensemble members should have identical gradients. Max diff: {max_diff}"
                )
    ...
    def test_reference_coordinate_broadcasting(self, ensemble_coords, mock_feats):
        ...
        energy_2d = potential_2d.compute_ensemble(
            coords, mock_feats, potential_2d.parameters
        )
        ...
        energy_3d = potential_3d.compute_ensemble(
            coords, mock_feats, potential_3d.parameters
        )
        ...
        )  # Ensure coords are 4D also

        energy_4d = potential_4d.compute_ensemble(
            coords, mock_feats, potential_4d.parameters
        )
    ...
    def test_denoising_selection_correctness(self, ensemble_coords, mock_feats):
        ...
        # Compute energies
        energy_all_sel = potential_all_selection.compute_ensemble(
            coords, mock_feats, potential_all_selection.parameters
        )
        energy_no_sel = potential_no_selection.compute_ensemble(
            coords, mock_feats, potential_no_selection.parameters
        )
        ...
        assert (
            torch.isfinite(energy_all_sel).all() and torch.isfinite(energy_no_sel).all()
        )
    ...
    def test_gradient_direction_consistency(self, ensemble_coords, mock_feats):
        ...
        potential = SubstructurePotential(
            parameters={
                ...
                "denoising_selection": np.array(
                    []
                ),  # No exclusions - all atoms constrained
        ...
        grad = potential.compute_gradient_ensemble(
            test_coords, mock_feats, potential.parameters
        )
        ...
        assert grad_member_0[0] > 0, (
            f"First member should have positive x gradient, got {grad_member_0[0]}"
        )
        assert grad_member_1[0] > 0, (
            f"Second member should have positive x gradient, got {grad_member_1[0]}"
        )
        ...
        assert max_diff < 1e-6, (
            f"Gradient directions should be identical for identical displacements, max diff: {max_diff}"
        )
    ...
    def test_gradient_direction_single_vs_ensemble(self, ensemble_coords, mock_feats):
        ...
        test_coords_ensemble = (
            test_coords_single.unsqueeze(1).expand(1, 2, -1, -1).contiguous()
        )
        ...
        mock_feats_ensemble = {
            "atom_pad_mask": torch.ones(
                2, n_atoms, dtype=torch.bool
            ),  # 2 ensemble members
        ...
        grad_single = potential_single.compute_gradient(
            test_coords_single, mock_feats_single, potential_single.parameters
        )
        grad_ensemble = potential_ensemble.compute_gradient_ensemble(
            test_coords_ensemble, mock_feats_ensemble, potential_ensemble.parameters
        )
        ...
        assert grad_diff_0.max() < 1e-6, (
            f"Single vs ensemble member 0 mismatch: {grad_diff_0.max()}"
        )
        assert grad_diff_1.max() < 1e-6, (
            f"Single vs ensemble member 1 mismatch: {grad_diff_1.max()}"
        )
    ...
    def test_gradient_magnitude_scaling(self, ensemble_coords, mock_feats):
        ...
        test_coords.requires_grad_(True)
        grad = potential.compute_gradient_ensemble(
            test_coords, mock_feats_3, potential.parameters
        )

        grad_mag_0 = torch.linalg.norm(
            grad[0, 0, 0, :]
        )  # Gradient magnitude for member 0
        grad_mag_1 = torch.linalg.norm(
            grad[0, 1, 0, :]
        )  # Gradient magnitude for member 1
        grad_mag_2 = torch.linalg.norm(
            grad[0, 2, 0, :]
        )  # Gradient magnitude for member 2

        print(
            f"Gradient magnitudes: {grad_mag_0:.3f}, {grad_mag_1:.3f}, {grad_mag_2:.3f}"
        )
        ...
        assert grad_mag_1 > grad_mag_0, (
            f"Member 1 should have larger gradient than member 0: {grad_mag_1} vs {grad_mag_0}"
        )
        assert grad_mag_2 > grad_mag_1, (
            f"Member 2 should have larger gradient than member 1: {grad_mag_2} vs {grad_mag_1}"
        )
    ...
    def test_ensemble_gradient_direction_issue(self):
        ...
        print(
            "Member 0 (x=+1.0) should have positive gradient (pointing away from origin)"
        )
        print(
            "Member 1 (x=-2.0) should have negative gradient (pointing away from origin)"
        )
        ...
        assert grad_mag_1 > grad_mag_0, (
            f"Member 1 should have larger gradient: {grad_mag_1} vs {grad_mag_0}"
        )
        ...
        assert direction_similarity < -0.5, (
            f"Gradient directions should be opposing, got {direction_similarity}"
        )
    ...
    def test_gradient_direction_reversal_bug(self):
        ...
        assert grad_1_x > grad_0_x, (
            f"Member 1 should have larger gradient than 0: {grad_1_x} vs {grad_0_x}"
        )
        assert grad_2_x > grad_1_x, (
            f"Member 2 should have larger gradient than 1: {grad_2_x} vs {grad_1_x}"
        )
