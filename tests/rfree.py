...
def parse_mtz_dump(
    dump_output, f_label="F", rfree_label="R-free-flags", extract_rfree=True
):
    """
    ...
    try:
        ...
        print(
            "Error: 'Column labels (one per line):' not found in dump output.",
            file=sys.stderr,
        )
        return {}, {}

    num_columns_line_idx = start_labels_idx - 1
    if num_columns_line_idx < 0 or not lines[num_columns_line_idx].startswith(
        "Number of columns:"
    ):
        print(
            "Error: Could not find 'Number of columns:' line before column labels.",
            file=sys.stderr,
        )
        return {}, {}
    ...
    for i in range(num_columns):
        if start_labels_idx + 1 + i < len(lines):
            ...
            print(
                "Error: Not enough column labels found in dump output.", file=sys.stderr
            )
            return {}, {}
    ...
    if f_val_idx is None:
        print(
            f"Error: No F column found. Tried labels: {possible_f_labels}",
            file=sys.stderr,
        )
    ...
    if extract_rfree:
        try:
            ...
            print(
                f"Warning: R-free label '{rfree_label}' not found. Will return empty rfree_flags.",
                file=sys.stderr,
            )
            rfree_val_idx = -1
    ...
    try:
        ...
        print(
            "Error: 'Column data (HKL followed by data):' not found in dump output.",
            file=sys.stderr,
        )
        return {}, {}
        ...
        for _ in range(num_columns):
            if current_line_idx < len(lines):
                try:
                    ...
                    print(
                        f"Warning: Could not parse data value at line {current_line_idx + 1}. Skipping reflection.",
                        file=sys.stderr,
                    )
                ...
                print(
                    f"Error: Unexpected end of file while reading data for HKL ({h},{k},{l}).",
                    file=sys.stderr,
                )
                break
...
def calculate_r_factors(f_obs_dict, f_calc_dict, rfree_flags):
    ...
    if not common_hkl:
        print(
            "Error: No common reflections found between the two MTZ files.",
            file=sys.stderr,
        )
    ...
    if rfree_flags:
        ...
        print(
            "Warning: No R-free flags found. Treating all reflections as work set.",
            file=sys.stderr,
        )
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: phenix.python rfree_between_mtz.py experiment.mtz comparison.mtz [f_label_exp] [f_label_comp]"
        )
        print("\nCalculates R-work and R-free between two MTZ files.")
        print(
            "  experiment.mtz  - MTZ file with 'experimental' F values and R-free flags"
        )
        ...
        print(
            "  f_label_comp    - F column label in comparison.mtz (default: searches common labels)"
        )
        print(
    ...
    try:
        ...
        f_obs, rfree_flags = parse_mtz_dump(
            dump_exp, f_label=f_label_exp, extract_rfree=True
        )
    ...
    if not f_obs:
        print(
            "No F_obs reflections found in experimental MTZ. Exiting.", file=sys.stderr
        )
        sys.exit(1)
    ...
    if r_work is not None:
        ...
        print(
            "Could not calculate R-factors due to insufficient data.", file=sys.stderr
        )
