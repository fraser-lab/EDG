...
def load_base_template(template_path: str) -> Dict[str, Any]:
    ...
    template_dict = {
        ...
        'structure': {
            ...
            'remove_all_ligands': getattr(template_config.structure, 'remove_all_ligands', False),
        ...
        'adaptive_solver': {
            ...
            'adaptive_line_search': getattr(template_config.adaptive_solver, 'adaptive_line_search', False),
    ...
    from edg.config.schedules import ParameterSchedule
    ...
    template_dict['density_guidance']['base_weight'] = convert_schedule_to_dict(template_config.density_guidance.base_weight)
    template_dict['density_guidance']['resampling_weight'] = convert_schedule_to_dict(template_config.density_guidance.resampling_weight)
    
    if hasattr(template_config.density_guidance, 'resolution') and template_config.density_guidance.resolution is not None:
        template_dict['density_guidance']['resolution'] = convert_schedule_to_dict(template_config.density_guidance.resolution)
    
    return template_dict


def generate_experiment_config(row: pd.Series, structure_base_path: str, 
                             output_base_dir: str, input_base_dir: str) -> Dict[str, Any]:
    ...
    synthetic_density_path = f"synthetic_density_output/{pdb_code}/{pdb_code}_main_2.0A.ccp4"
    ...
    exp_config = {
        'name': pdb_code,
        'structure': {
            'structure_path': f"{structure_base_path}/{pdb_code}.cif"
        },
        'density': {
...
def main():
    ...
    parser.add_argument(
        "--csv", 
        required=True,
        help="Path to altloc_summary.csv file"
    )
    parser.add_argument(
        "--template", 
        required=True,
    ...
    parser.add_argument(
        "--output", 
        required=True,
        help="Path to output batch configuration YAML file"
    )
    parser.add_argument(
        "--structure-base-path",
    ...
    for _, row in df.iterrows():
        try:
            exp_config = generate_experiment_config(
                row, args.structure_base_path, 
                args.output_base_dir, args.input_base_dir
            )
            experiments.append(exp_config)
            print(f"  Generated config for {row['pdb_code']}: chain {row['target_chain']} resi {row['segment_start']}-{row['segment_end']}")
