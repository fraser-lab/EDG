...
def create_shared_input_directory(
    config: ExperimentConfig, base_results_dir: Path
) -> Tuple[Path, Path]:
...
def validate_shared_input_compatibility(
    config: ExperimentConfig, shared_input_dir: Path
) -> bool:
    ...
    try:
        ...
        if not metadata_file.exists():
            logger.warning(
                f"No metadata file found in shared input directory: {shared_input_dir}"
            )
            return False
        ...
        if stored_structure_path != config.structure.structure_path:
            logger.warning(
                f"Structure path mismatch: {config.structure.structure_path} vs {stored_structure_path}"
            )
            return False
        ...
        if stored_model_version != config.model.version:
            logger.warning(
                f"Model version mismatch: {config.model.version} vs {stored_model_version}"
            )
            return False
        ...
        if stored_boltz_yaml != config.boltz_input_yaml:
            logger.warning(
                f"Boltz input YAML mismatch: {config.boltz_input_yaml} vs {stored_boltz_yaml}"
            )
            return False
...
def copy_boltz_input_to_shared(
    config: ExperimentConfig, shared_input_dir: Path
) -> Path:
...
def create_config_with_shared_input(
    config: ExperimentConfig,
    shared_input_dir: Path,
    shared_output_dir: Path,
    run_output_dir: Path,
