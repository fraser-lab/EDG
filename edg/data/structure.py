...
class Structure(_BaseStructure):
    ...
    def complete_residues(self):
        ...
        for residue in structure.residues:
            ...
            if expected_atoms:
                missing_atoms = [
                    atom for atom in expected_atoms if atom not in residue.name
                ]
                if missing_atoms:
                    print(
                        f"Completing residue {residue.resn[0]} {residue.resi[0]} - missing {len(missing_atoms)} atoms: {missing_atoms}"
                    )
                ...
                print(
                    f"Completing residue {residue.resn[0]} {residue.resi[0]} with inactive atoms"
                )
                initial_count = len(residue.name)
                residue.complete_residue()
                final_count = len(residue.name)
        ...
        if completed_residues:
            print(
                f"Rebuilding structure to incorporate atoms from {len(completed_residues)} completed residues"
            )
    ...
    def _rebuild_structure_from_residues(self):
                ...
                for attr in all_attributes:
                    ...
                    if hasattr(residue, attr):
                        ...
                        if hasattr(attr_data, "__len__") and len(attr_data) > i:
                            ...
                            value = (
                                attr_data
                                if not hasattr(attr_data, "__len__")
                                else (attr_data[0] if len(attr_data) > 0 else None)
                            )
                    else:
                        ...
                        if attr == "record":
                            ...
                            f"u{i}{j}" for i in "012" for j in "012" if i <= j
                        ]:
                            value = 0.0
                        elif attr == "active":
                            value = True
        ...
        for attr in new_data:
            ...
            try:
                ...
                print(
                    f"Warning: Could not convert {attr} to {target_dtype}, using default dtype: {e}"
                )
                new_data[attr] = np.array(new_data[attr])

        print(
            f"Rebuilt structure with {len(new_data['name'])} atoms (was {len(self.name)})"
        )
