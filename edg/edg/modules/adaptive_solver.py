...
class AdamGradientSolver(AdaptiveGradientSolver):
    ...
    def step(
        ...
        stats["initial_energy"] = (
            initial_energy.item()
            if hasattr(initial_energy, "item")
        ...
        for iteration in range(self.config.max_iterations):
            ...
            for potential in potentials:
                ...
                if (
                    ...
                    and iteration % parameters["guidance_interval"] == 0
                ):
                    grad = compute_gradient_fn(
                        potential, current_coords, feats, parameters
                    )
                    ...
                    if self.config.per_potential_scaling:
                        ...
                        if potential_name not in self.potential_scales:
                            ...
                            self.potential_scales[potential_name] = (
                                alpha * grad_norm
                                + (1 - alpha) * self.potential_scales[potential_name]
                            )

                        # Scale gradient
                        if self.potential_scales[potential_name] > 1e-8:
                            grad = grad / (
                                self.potential_scales[potential_name] + self.config.eps
                            )
            ...
            if (
                ...
                and grad_norm > self.config.gradient_clip_norm
            ):
                total_gradient = total_gradient * (
                    self.config.gradient_clip_norm / grad_norm
                )
            ...
            self.m = (
                self.config.beta1 * self.m + (1 - self.config.beta1) * total_gradient
            )

            # Update biased second moment estimate
            self.v = self.config.beta2 * self.v + (1 - self.config.beta2) * (
                total_gradient**2
            )
    ...
    def _backtracking_line_search(
        ...
        grad_dot_dir = torch.sum(
            direction * direction
        ).item()  # For descent, this should be positive
        ...
        for _ in range(self.config.max_line_search_steps):
            ...
            if (
                new_energy
                <= initial_energy - self.config.line_search_c1 * alpha * grad_dot_dir
            ):
                return alpha
    ...
    def _adaptive_backtracking_line_search(
        ...
        for step in range(self.config.max_line_search_steps):
            ...
            if abs(armijo_threshold) > 1e-12:
                violation_ratio = (new_energy - armijo_threshold) / abs(
                    armijo_threshold
                )
            else:
                violation_ratio = 1.0
            ...
            adaptive_factor = max(
                ...
                min(
                    ...
                    self.config.line_search_backtrack
                    * (1.0 + self.config.violation_scaling * violation_ratio),
                ),
            )
                ...
                if (
                    ...
                    if self.config.per_potential_scaling:
                        ...
                        if potential_name not in self.potential_scales:
                            ...
                            self.potential_scales[potential_name] = (
                                0.9 * self.potential_scales[potential_name]
                                + 0.1 * grad_norm
                            )
...
def create_adaptive_solver(
    solver_type: str = "adam", config: Optional[AdaptiveSolverConfig] = None
